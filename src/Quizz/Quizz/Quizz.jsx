...
const Quiz = ({ chosenNum }) => {
    const { maData, addOrUpdateResponse } = useContext(QuizzContext);
    const { questions } = quizz;

    // Filtrer les questions en fonction de chosenNum
    const numOfQuestions = questions.slice(0, chosenNum);
    ...
    const onClickValidate = () => {
        ...
        setCorrectAnswer(numOfQuestions[activeQuestion].correctAnswer);
    };
    ...
    const onClickNext = () => {
        ...
        if (activeQuestion !== numOfQuestions.length - 1) {
    ...
    const handleResponse = () => {
        const questionId = numOfQuestions[activeQuestion].question_id;
        const response = {
        question_id: questionId,
        user_id: 101,
        text: userAnswer,
        evalIndex: selectedEvaluationIndex,
        questions: numOfQuestions[activeQuestion]
        };
    ...
    const maxScore = 2 * numOfQuestions.length;
    ...
    return (
        ...
        {!showResult ? (
            ...
            <div style={{ width: '1em', position: 'relative', top: '8px', left: '8px' }}>
                ...
                <span>/{addLeadingZero(numOfQuestions.length)}</span>
            </div>
            <h2>{numOfQuestions[activeQuestion].question}</h2>
            <div style={{ marginBottom: '1rem' }}>
                Question {activeQuestion + 1} sur {numOfQuestions.length}
            </div>
            ...
            {!showValidateButton && (
                ...
                <div className="flex-right">
                    ...
                    variant='outlined'
                    >
                    {activeQuestion === numOfQuestions.length - 1 ? 'Terminer' : 'Question suivante'}
                    </Button>
                </div>
                </div>
            )}
            ...
            <h3>Résultat</h3>
            <p>
                Nombre total de questions : <span>{numOfQuestions.length}</span>
            </p>
            ...
            <ul>
                ...
                <li>
                    ...
                    <div
                        ...
                        style={{ width: `${(scores.filter(score => score === 0).length / numOfQuestions.length) * 100}%` }}
                ...
                <li>
                    ...
                    <div
                        ...
                        style={{ width: `${(scores.filter(score => score === 1).length / numOfQuestions.length) * 100}%` }}
                ...
                <li>
                    ...
                    <div
                        className="acquise"
                        style={{ width: `${(scores.filter(score => score === 2).length / numOfQuestions.length) * 100}%` }}
